...
# GPTQ (Frantar et al., 2022) is a PTQ method that minimises the
# layer-wise reconstruction error using second-order information.
# Result: near-lossless INT4 for 7B+ models.

print("""
GPTQ Algorithm (conceptual):
─────────────────────────────
For each transformer layer:
  1. Collect activations on a small calibration dataset (128 samples)
  2. Compute the Hessian H = E[x x^T] for each weight matrix
  3. Quantise weights column by column, updating remaining columns
     to compensate for the quantisation error using H^{-1}

Key insight: errors in one weight can be partially corrected by
adjusting neighbouring weights — requires access to activations.

Usage (AutoGPTQ library):
