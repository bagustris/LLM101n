...
print("""
ZeRO (Zero Redundancy Optimizer) Memory Budget per GPU
═══════════════════════════════════════════════════════

For a model with Φ parameters trained in mixed precision:
  Baseline DDP memory per GPU: 16Φ bytes
    ├─ FP16 params:          2Φ
    ├─ FP16 grads:           2Φ
    └─ Optim state (FP32):  12Φ  (fp32 params + m + v for AdamW)

  ZeRO Stage 1 (shard optimiser state):   (4 + 12/N)Φ bytes
  ZeRO Stage 2 (shard grads too):         (2 + 14/N)Φ bytes
  ZeRO Stage 3 (shard everything):        (16/N)Φ bytes

where N = number of GPUs.

Example: 7B model, N=8 GPUs
  Baseline : 16 × 7e9 × 1 byte = 112 GB per GPU (won't fit on 80 GB A100!)
  ZeRO-3   : 16 × 7e9 / 8 = 14 GB per GPU  ← fits!
""")
